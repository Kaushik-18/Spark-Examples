...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val remainingFields = columnNames.tail.map(x => StructField(x, DoubleType, nullable = false))
    StructType(firstField :: remainingFields)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    classifyColumnsRec(columnNames, primary, leisure, working)
  }
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = {
      when($"tesex" === 1.0, "male").otherwise("female").as("sex")
    }
    val ageProjection: Column = {
      when($"teage" >= 15 && $"teage" <= 22, "young").when($"teage" >= 23 && $"teage" <= 55, "active").otherwise("elder").as("age")
    }
